...
export default function PublicAdoptPage() {
  const supabase = createClient();
  const [animals, setAnimals] = useState<any[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  // States for the Search and Filter
  ...
  // Filter the grid based on search text AND dropdown selection
  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-4 md:px-8">
      ...
      {/* Header section (Updated to welcome both cats and dogs) */}
      <div className="mb-8">
        <h1 className="font-serif text-4xl text-sol-dark font-bold mb-1">Find your forever companion</h1>
        <p className="text-sol-dark/60 text-sm">{filteredAnimals.length} animals available for adoption</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-12">
        ...
        <select 
          ...
          className="px-4 py-3 rounded-xl border border-sol-dark/10 bg-white min-w-[200px] focus:outline-none focus:border-sol-yellow focus:ring-1 focus:ring-sol-yellow transition-all cursor-pointer text-sm"
        >
      ...
      {isLoading ? (
        <div className="text-center text-sol-dark/50 py-12">Loading furry friends...</div>
              ...
              <div key={animal.animal_id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-sol-dark/10 hover:shadow-md transition-shadow flex flex-col h-full">
                ...
                <div className="h-56 bg-sol-dark relative overflow-hidden flex-shrink-0">
                  {photoUrl ? (
                    ...
                    // Paw placeholder for animals without photos
                    <div className="w-full h-full flex items-center justify-center text-sol-yellow/30">
                      <i className="ti ti-paw text-5xl"></i>
                    </div>
                  )}
                  {/* Status Tag */}
                  <span className="absolute top-4 left-4 bg-sol-yellow text-sol-dark text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wide">
                    {animal.adoption_status === 'available' ? 'Available' : 'On Hold'}
                  </span>
                </div>
